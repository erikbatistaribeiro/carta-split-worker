...
async function buscarDadosCard(cardId, token) {
  const query = `query { card(id: "${cardId}") { fields { name value array_value } } }`;
  ...
  for (const f of card.fields || []) {
    // array_value cobre campos de texto longo que o Pipefy trunca no value
    const val = f.value || (Array.isArray(f.array_value) ? f.array_value.join("\n") : null);
    if (val) c[f.name] = val;
  }
...
async function gerarPdf(d) {
  ...
  const FS  = 12;
  const LS  = FS * 1.5;
  const MB  = 71; // margem base
  const K   = rgb(0, 0, 0);

  // Página atual e cursor Y
  let page = doc.addPage([PAGE_W, PAGE_H]);
  let y    = PAGE_H - MT;

  // Garante espaço ou cria nova página
  function checkY(needed = LS) {
    if (y - needed < MB) {
  ...
  function para(text, sp = 0) {
    ...
    for (const l of wrap(san(text), fR, FS, CONTENT_W)) {
      checkY();
      page.drawText(san(l), { x: ML, y, size: FS, font: fR, color: K });
      y -= LS;
    }
  }
  ...
  // Boletos — suporta qualquer quantidade com paginação automática
  for (const b of d.dadosBoleto) {
    checkY(LS * 5); // garante espaço para pelo menos o cabeçalho do boleto
    line(b.razao, true);
  ...
  // Assinatura — garante que fique na mesma página com espaço suficiente
  checkY(LS * 5);
  y -= LS * 2;
  page.drawLine({
